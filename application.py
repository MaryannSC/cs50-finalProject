...
def logSwims():
    ...
    if request.method == "POST":
        ...
        #print("Today's date: {}, type: {}".format(today,type(today)))
        ...
        #print("Entered date: {}, type: {}".format(swimDate,type(swimDate)))
        if swimDate > today:
        ...
        #print('Date: {}, Distance: {}, Units: {}'.format(swimDate, distance, units))
        # Date: 2021-10-16, Distance: 2300, Units: yards
...
def monthlyResults():
    ...
    #return apology("Coming Soon", 400)
...
def preferences():
    ...
    if request.method == "POST":
        ...
        #print('from preferences: Units = {}'.format(units))

        # Enter units preference into database
        db.execute("UPDATE users SET units=? WHERE userid=?", units, session["user_id"])

        # read back
        #userInfo = db.execute("SELECT * FROM users WHERE userid=?", session["user_id"])
        #print('from preferences: userInfo = {}'.format(userInfo))
...
def login():
    ...
    if request.method == "POST":
        ...
        #print("Logged In:")
        #print("Session: user_id: {}, units: {}".format(session["user_id"], session["units"]))
...
# From index()
    # if units == 'yards':
    #     row = db.execute("SELECT strftime('%Y', date) as year, SUM(yards) FROM swims WHERE userid=? AND year=?", session["user_id"], year)
    #     total = row[0]['SUM(yards)']
    # elif units == 'meters':
    #     row = db.execute("SELECT strftime('%Y', date) as year, SUM(meters) FROM swims WHERE userid=? AND year=?", session["user_id"], year)
    #     total = row[0]['SUM(meters)']
    # elif units == 'miles':
    #     row = db.execute("SELECT strftime('%Y', date) as year, SUM(miles) FROM swims WHERE userid=? AND year=?", session["user_id"], year)
    #     total = row[0]['SUM(miles)']
    # else:
    #     return apology("Error with summing units", 400)

    # # format display of total distance - round to 2 decimal places and use commas
    # total = "{:,.2f}".format(total)

# from monthlyResults
    # # get monthly totals
    # if units == 'yards':
    #     monthlyTotals = db.execute("SELECT strftime('%Y', date) as year, strftime('%m', date) as month, SUM(yards) "
    #                               "FROM swims WHERE userid=? GROUP BY year, month ORDER BY year DESC, month DESC", session["user_id"])
    #     for month in monthlyTotals:
    #         month['distance'] = f"{month['SUM(yards)']:,.2f}"

    #     yearTotals = db.execute("SELECT strftime('%Y', date) as year, SUM(yards) FROM swims WHERE userid=? "
    #                             "GROUP BY year ORDER BY year DESC", session["user_id"])

    #     for year in yearTotals:
    #         year['distance'] = f"{year['SUM(yards)']:,.2f}"

    # elif units == 'meters':
    #     monthlyTotals = db.execute("SELECT strftime('%Y', date) as year, strftime('%m', date) as month, SUM(meters) "
    #                               "FROM swims WHERE userid=? GROUP BY year, month ORDER BY year DESC, month DESC", session["user_id"])
    #     for month in monthlyTotals:
    #         month['distance'] = f"{month['SUM(meters)']:,.2f}"

    #     yearTotals = db.execute("SELECT strftime('%Y', date) as year, SUM(meters) FROM swims WHERE userid=? "
    #                             "GROUP BY year ORDER BY year DESC", session["user_id"])

    #     for year in yearTotals:
    #         year['distance'] = f"{year['SUM(meters)']:,.2f}"

    # elif units == 'miles':
    #     monthlyTotals = db.execute("SELECT strftime('%Y', date) as year, strftime('%m', date) as month, SUM(miles) "
    #                               "FROM swims WHERE userid=? GROUP BY year, month ORDER BY year DESC, month DESC", session["user_id"])
    #     for month in monthlyTotals:
    #         month['distance'] = f"{month['SUM(miles)']:,.2f}"

    #     yearTotals = db.execute("SELECT strftime('%Y', date) as year, SUM(miles) FROM swims WHERE userid=? "
    #                             "GROUP BY year ORDER BY year DESC", session["user_id"])

    #     for year in yearTotals:
    #         year['distance'] = f"{year['SUM(miles)']:,.2f}"

    # else:
    #     return apology("Error with summing units", 400)

    # print("monthlyTotals = {}".format(monthlyTotals))
    # print("yearTotals = {}".format(yearTotals))
